...
var firstTreekAchieved = false
var secondTreekAchieved = false
var thirdTreekAchieved = false

class foodActionVC: UIViewController{
    var timer = Timer()
    var shouldSegueBePerformed = false
    ...
    func layoutAllViews(){
        ...
        foodLabel.frame = CGRect(x:0.39371980676328505*self.view.frame.width, y:0.18973214285714285*self.view.frame.height, width:0.21256038647342995*self.view.frame.width,height:0.04575892857142857*self.view.frame.height)
        
        foodLabel.adjustsFontSizeToFitWidth = true
        foodLabel.minimumScaleFactor = 0.1
        foodLabel.numberOfLines = 0
        foodLabel.lineBreakMode = .byClipping
    ...
    @objc func performSegToFoodTable(){
        ...
        if shoudlSegueBePerformedForFoodTable == true{
            performSegue(withIdentifier: "goToFoodTable", sender: self)
            shoudlSegueBePerformedForFoodTable = false
            timer.invalidate()
        }
    }
